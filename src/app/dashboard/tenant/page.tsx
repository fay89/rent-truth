...
export default function TenantDashboard() {
    const { user } = useAuth();
    const { getContractsByTenant, verifyContract, reviews } = useData();
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-neutral-500">Bienvenido a tu panel de control.</p>
                </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                        ...
                        <Card className="border-l-4 border-l-yellow-500 shadow-sm bg-yellow-50/50">
                            <CardHeader>
                                <CardTitle className="text-yellow-700 flex items-center gap-2">
                                    <Clock className="w-5 h-5" /> Acciones Pendientes
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                {pendingContracts.map(contract => (
                                    <div key={contract.id} className="bg-white p-4 rounded-lg border border-yellow-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                                        <div>
                                            <p className="font-medium text-brand-blue">Contrato pendiente de firma</p>
                                            <p className="text-sm text-neutral-500">{contract.propertyAddress}</p>
                                            <p className="text-xs text-neutral-400">Propietario: {contract.landlordId}</p>
                                        </div>
                                        <Button
                    ...
                    <Card className="border-none shadow-sm">
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Tus alquileres activos e históricos.</CardDescription>
                            </div>
                            ...
                            {contracts.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    {contracts.slice(0, 3).map((contract) => (
                                        <div key={contract.id} className="group flex items-center justify-between p-4 rounded-lg border border-neutral-100 bg-white hover:border-brand-blue/20 hover:shadow-md transition-all">
                                            <div className="flex items-center gap-4">
                                                <div className="h-10 w-10 rounded-full bg-blue-50 flex items-center justify-center text-brand-blue">
                                                    <FileText className="w-5 h-5" />
                                                </div>
                                                <div>
                                                    <p className="font-medium text-brand-blue group-hover:text-brand-green transition-colors">{contract.propertyAddress}</p>
                                                    <p className="text-xs text-neutral-500">{contract.startDate} - {contract.endDate}</p>
                                                </div>
                                            </div>
                    ...
                    {!hasContract && (
                        <div className="bg-brand-blue text-white rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-lg">
                            <div>
                                <h3 className="text-xl font-bold mb-2">Sube tu primer contrato</h3>
                                <p className="text-blue-100 text-sm max-w-md">Empieza a generar tu historial de reputación verificada. Es gratuito y seguro.</p>
                            </div>
                            <Button className="bg-white text-brand-blue hover:bg-blue-50 whitespace-nowrap">
                                Crear contrato
                            </Button>
                        </div>
                    )}
                    ...
                    <Card className="border-none shadow-sm">
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex gap-3 items-start">
                                ...
                                <div>
                                    <p className="text-sm font-medium">Bienvenido a RentTruth</p>
                            ...
                            <div className="flex gap-3 items-start opacity-60">
                                ...
                                <div>
                                    <p className="text-sm font-medium">Completa tu perfil para ser verificado</p>
                    ...
                    <Card className="border-none shadow-sm bg-gradient-to-br from-blue-50 to-white">
                        ...
                        <CardContent className="space-y-3">
                            <div className="bg-white p-3 rounded-md border border-blue-100 shadow-sm">
                                ...
                                <p className="text-xs text-neutral-500">Los perfiles verificados tienen un 80% más de probabilidad de ser aceptados.</p>
                            </div>
                            <div className="bg-white p-3 rounded-md border border-blue-100 shadow-sm">
                                <p className="text-sm text-brand-blue font-medium mb-1">Pago puntual</p>
                                <p className="text-xs text-neutral-500">Paga tu alquiler antes del día 5 para mantener un score perfecto.</p>
                            </div>
                        </CardContent>
                    </Card>
